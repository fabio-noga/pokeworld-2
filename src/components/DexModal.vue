<template>
  <Teleport to="body">
    <div class="dex-overlay" @click.self="$emit('close')">
      <div class="dex-panel">

        <!-- Modal header strip -->
        <div class="dex-modal-bar">
          <img src="/textures/Nav/Pokedex/0.png" class="dex-modal-icon" alt="" />
          <span class="dex-modal-title">POKÉDEX</span>
          <button class="dex-close-btn" @click="$emit('close')" title="Close">✕</button>
        </div>
        ...
        <div class="dex-wrap">
          <div class="bento-grid">
            ...
            <!-- ② PREV -->
            <button class="bento-cell cell-prev" :disabled="selectedId <= 1" @click="selectPokemon(selectedId - 1)"
              :aria-label="selectedId > 1 ? `Previous: ${pokedex(selectedId - 1)}` : 'No previous'">
              <span class="nav-arrow">◀</span>
              <template v-if="selectedId > 1">
                <img class="nav-sprite" :src="`/textures/Mini/Png/${padId(selectedId - 1)}.png`"
            ...
            <div class="bento-cell cell-hero" :style="selectedKnown ? heroStyle : {}">
              <img class="hero-gif" :src="`/textures/Mini/Gif/${padId(selectedId)}.gif`" alt="" width="48" height="48"
            ...
            <!-- ④ NEXT -->
            <button class="bento-cell cell-next" :disabled="selectedId >= 151" @click="selectPokemon(selectedId + 1)"
                ...
                <template v-if="selectedCaught">
                  <div v-for="entry in learnsetRows" :key="entry.id" class="lt-row" role="row">
                    ...
                    <span class="lt-type" role="cell">
                      <span class="type-badge-sm" :style="{ backgroundColor: TYPE_COLORS[entry.type] ?? '#888' }">{{ TYPE_NAMES[entry.type] ?? '???' }}</span>
                    </span>
...
onMounted(() => {
  ...
  document.querySelector('.dex-list-scroll .list-item.active')?.scrollIntoView({ block: 'center' })
})
onUnmounted(() => {
...
function isKnown(id: number): boolean {
  return !tpeMode.value || !!saveStore.pokedex[String(id)]
}
...
// ── Selected Pokémon (local ref instead of route) ────────────────
...
const artStyle  = computed(() => ({ borderTopColor: primaryColor.value, borderTopWidth: '4px' }))
...
.dex-close-btn:hover { background: rgba(255,255,255,0.3); }
...
.cell-list     { grid-column: 1 / 3;  grid-row: 1 / 10; display: flex; flex-direction: column; align-self: start; position: sticky; top: 56px; max-height: calc(94vh - 80px); }
...
/* ── List ── */
.list-head { display: flex; align-items: center; justify-content: space-between; padding: 10px 10px 8px; background: #c82020; border-bottom: 3px solid #881010; flex-shrink: 0; }
.list-title-row { display: flex; align-items: center; gap: 8px; }
.list-title { font-size: 11px; font-weight: 700; color: #fff; letter-spacing: 0.5px; text-shadow: 1px 1px 0 #660808; }
.list-counts { font-family: 'VT323', monospace; font-size: 14px; display: flex; flex-direction: column; align-items: flex-end; gap: 1px; }
...
.tpe-btn { background: rgba(255,255,255,0.15); border: 1px solid rgba(255,255,255,0.3); border-radius: 4px; color: rgba(255,255,255,0.5); width: 24px; height: 24px; display: flex; align-items: center; justify-content: center; cursor: pointer; font-size: 11px; transition: background 0.15s, color 0.15s; }
...
.list-search-bar { padding: 6px 8px; border-bottom: 1px solid #b8d4e8; background: #e8f4fc; flex-shrink: 0; }
.list-input { width: 100%; background: #fff; border: 2px solid #a0c0d8; border-radius: 2px; color: #181830; font-family: 'VT323', monospace; font-size: 15px; padding: 4px 6px; outline: none; transition: border-color 0.15s; }
...
.list-item { display: flex; align-items: center; gap: 5px; padding: 4px 8px; cursor: pointer; border-bottom: 1px solid #d0e8f4; min-height: 32px; transition: background 0.1s; touch-action: manipulation; }
...
/* ── Prev / Next ── */
.cell-prev, .cell-next { display: flex; align-items: center; justify-content: center; gap: 6px; padding: 8px; text-align: center; border: none; outline: none; transition: background 0.15s, box-shadow 0.1s, transform 0.1s; touch-action: manipulation; }
.cell-prev:hover:not(:disabled), .cell-next:hover:not(:disabled) { background: #d8eef8; }
.cell-prev:active:not(:disabled), .cell-next:active:not(:disabled) { transform: translateY(3px); box-shadow: 0 2px 0 #1a3080; }
.cell-prev:disabled, .cell-next:disabled { opacity: 0.25; cursor: not-allowed; box-shadow: none; }
.cell-prev { flex-direction: row; justify-content: flex-start; }
.cell-next { flex-direction: row; justify-content: flex-end; }
.nav-arrow { font-size: 12px; font-weight: 700; color: #2848a8; flex-shrink: 0; line-height: 1; }
.nav-sprite { width: 36px; height: 36px; image-rendering: pixelated; flex-shrink: 0; }
.nav-info { display: flex; flex-direction: column; gap: 1px; min-width: 0; }
.nav-info-l { align-items: flex-start; } .nav-info-r { align-items: flex-end; }
.nav-name { font-size: 11px; font-weight: 600; color: #181830; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 90px; line-height: 1; }
.nav-num  { font-family: 'VT323', monospace; font-size: 13px; color: #6080a0; line-height: 1; }

/* ── Hero ── */
.cell-hero { display: flex; align-items: center; gap: 12px; padding: 12px 16px; }
...
.hero-name { font-size: 21px; font-weight: 700; color: #181830; margin: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.hero-num  { font-family: 'VT323', monospace; font-size: 20px; color: #6080a0; flex-shrink: 0; }
.hero-types { display: flex; gap: 5px; flex-wrap: wrap; }
.type-badge { font-size: 10px; font-weight: 700; color: #fff; padding: 3px 9px; border-radius: 3px; text-shadow: 0 1px 0 rgba(0,0,0,0.4); letter-spacing: 0.3px; }
.dex-status { font-size: 10px; font-weight: 600; padding: 4px 9px; border-radius: 3px; border: 2px solid; letter-spacing: 0.2px; }
...
/* ── Art / TV ── */
.cell-art { display: flex; flex-direction: column; background: transparent; border: none; border-radius: 0; box-shadow: none; overflow: visible; }
.art-tv { flex: 1; background: linear-gradient(160deg, #252838 0%, #181a28 100%); border-radius: 8px; box-shadow: 0 4px 0 #0d0e1a; display: flex; flex-direction: column; overflow: hidden; }
...
.art-tv-screen { margin: 9px 9px 0; border-radius: 4px; overflow: hidden; background: #fff; display: flex; flex-direction: column; position: relative; box-shadow: inset 0 0 0 1px rgba(0,0,0,0.1), inset 2px 2px 10px rgba(0,0,0,0.06); animation: crt-flicker 6s infinite; }
...
.art-dialogue { position: absolute; bottom: 6px; left: 6px; right: 6px; z-index: 5; padding: 7px 10px 9px; background: #fff; border: 2px solid #181830; border-radius: 5px; box-shadow: 0 0 0 2px #fff, 0 0 0 4px #181830; }
.dlg-toggle { position: absolute; top: -14px; right: 0; width: 20px; height: 14px; background: #fff; border: 2px solid #181830; border-bottom: none; border-radius: 3px 3px 0 0; color: #181830; font-size: 7px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; padding: 0; transition: background 0.15s, color 0.15s; }
.dlg-toggle:hover { background: #e8f0fc; }
.dlg-show-tab { position: absolute; bottom: 6px; right: 6px; width: 22px; height: 16px; background: #181830; border: 2px solid #181830; border-radius: 3px; color: #fff; font-size: 7px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; padding: 0; z-index: 5; transition: background 0.15s; }
.dlg-show-tab:hover { background: #2848a8; border-color: #2848a8; }
.art-dialogue-text { font-family: 'VT323', monospace; font-size: 15px; line-height: 1.55; color: #181830; margin: 0; }
.art-dlg-cur { color: #c82020; margin-left: 3px; }
.art-bezel-bar { display: flex; align-items: center; justify-content: space-between; padding: 6px 11px 8px; }
...
.art-tv-screen.is-off .art-tv-crt-wrap { transform: scaleY(0.03); filter: brightness(4); opacity: 0; transition: transform 0.18s ease-in, opacity 0.1s 0.08s ease-in, filter 0.06s ease-in; }
.art-led { width: 6px; height: 6px; border-radius: 50%; background: #c82020; box-shadow: 0 0 5px #c82020, 0 0 2px #ff4040; flex-shrink: 0; cursor: pointer; transition: background 0.2s, box-shadow 0.2s; }
.art-led.led-off { background: #3a0808; box-shadow: none; }
.art-speaker { display: flex; gap: 3px; align-items: center; }
.art-speaker span { display: block; width: 2px; height: 9px; background: rgba(255,255,255,0.18); border-radius: 1px; }
.art-model { font-family: 'VT323', monospace; font-size: 11px; color: rgba(255,255,255,0.22); letter-spacing: 1.5px; flex-shrink: 0; }

/* ── Stats ── */
.cell-stats { padding: 14px 16px; }
.cell-heading { font-size: 11px; font-weight: 700; color: #c82020; letter-spacing: 0.5px; margin-bottom: 12px; }
.stat-row { display: flex; align-items: center; gap: 8px; margin-bottom: 10px; }
.stat-lbl { font-size: 10px; font-weight: 600; color: #6080a0; min-width: 36px; flex-shrink: 0; letter-spacing: 0.3px; }
.stat-val { font-family: 'VT323', monospace; font-size: 17px; color: #181830; min-width: 32px; text-align: right; flex-shrink: 0; }
.stat-bar-bg { flex: 1; height: 9px; background: #d0e4f0; border-radius: 2px; overflow: hidden; border: 1px solid #b0c8e0; }
.stat-bar-fill { height: 100%; border-radius: 2px; transition: width 0.4s ease-out; }
.stats-total { font-size: 11px; font-weight: 600; color: #6080a0; margin-top: 8px; text-align: right; letter-spacing: 0.3px; }
.stats-total-val { color: #181830; margin-left: 6px; }

/* ── Evo ── */
.cell-evo { display: flex; flex-direction: column; align-items: flex-start; gap: 6px; padding: 10px 16px; overflow-x: auto; }
.evo-chain { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
.evo-chain-shiny { background: linear-gradient(90deg, rgba(255,220,80,0.08), rgba(255,180,240,0.08)); border-radius: 6px; padding: 4px 6px; }
.evo-divider { width: 100%; display: flex; align-items: center; gap: 6px; margin: 2px 0; }
.evo-divider span { font-family: 'Press Start 2P', monospace; font-size: 7px; color: #2848a8; letter-spacing: 0.5px; opacity: 0.7; }
.evo-divider::after { content: ''; flex: 1; height: 1px; background: #c0d4f0; }
.evo-eevee { flex-wrap: nowrap; align-items: flex-start; }
.evo-branches { display: flex; flex-direction: column; gap: 4px; }
.evo-branch { display: flex; align-items: center; gap: 6px; }
.evo-stage { display: flex; flex-direction: column; align-items: center; gap: 2px; padding: 4px 8px; background: none; border: 2px solid transparent; border-radius: 6px; cursor: pointer; transition: background 0.15s, border-color 0.15s; text-decoration: none; }
...
.evo-sprite { image-rendering: pixelated; width: 32px; height: 32px; }
.evo-name { font-family: 'VT323', monospace; font-size: 13px; color: #181830; line-height: 1; white-space: nowrap; }
.evo-connector { display: flex; flex-direction: column; align-items: center; gap: 1px; flex-shrink: 0; }
.evo-arr  { font-size: 10px; color: #4060a0; line-height: 1; }
.evo-trig { font-family: 'VT323', monospace; font-size: 12px; color: #4060a0; white-space: nowrap; line-height: 1; }

/* ── Learnset ── */
.cell-heading-red { display: flex; align-items: center; justify-content: space-between; padding: 10px 14px; background: #c82020; border-bottom: 3px solid #881010; font-size: 11px; font-weight: 700; color: #fff; letter-spacing: 0.5px; text-shadow: 1px 1px 0 #660808; }
.section-count { font-family: 'VT323', monospace; font-size: 15px; color: #ffc8c8; font-weight: normal; }
.learnset-table { overflow-x: auto; }
.lt-head, .lt-row { display: grid; grid-template-columns: 44px 1fr 80px 52px 44px 52px; align-items: center; padding: 0 12px; border-bottom: 1px solid #d0e8f4; }
.lt-head { font-size: 10px; font-weight: 700; color: #6080a0; padding-top: 8px; padding-bottom: 8px; letter-spacing: 0.3px; background: #e8f4fc; border-bottom: 2px solid #b0cce0; }
.lt-row { padding-top: 6px; padding-bottom: 6px; background: #fff; transition: background 0.1s; }
...
.lt-pwr { color: #c04020; }
.type-badge-sm { font-size: 9px; font-weight: 700; color: #fff; padding: 2px 6px; border-radius: 3px; text-shadow: 0 1px 0 rgba(0,0,0,0.5); white-space: nowrap; }
.lt-empty { font-family: 'VT323', monospace; font-size: 18px; color: #a0c0d8; text-align: center; padding: 24px; background: #fff; }
.lt-catch-hint { font-family: 'VT323', monospace; font-size: 17px; color: #8090b0; font-style: italic; text-align: center; padding: 20px 12px; background: #fff; border-bottom: 1px solid #d0e8f4; }

/* ── Responsive ── */
@media (max-width: 900px) {
  .bento-grid { grid-template-columns: 80px 80px 1fr 1fr 1fr; grid-template-rows: auto auto auto auto auto; }
  .cell-list     { grid-column: 1 / 3; grid-row: 1 / 6; }
  .cell-prev     { grid-column: 3 / 4; grid-row: 1; }
  .cell-hero     { grid-column: 4 / 6; grid-row: 1; }
  .cell-next     { grid-column: 3 / 4; grid-row: 2; }
